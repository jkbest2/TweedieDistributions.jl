...
cpg2_ci = cdf(Normal(), 0.995) *
    sqrt(failprob(CPG2) * succprob(CPG2) / n)
cpg1_lpdf = logpdf(CPG1, cpg1_rand)
cpg0_lpdf = logpdf(CPG2, 0.0)
...
@testset "TweedieDistributions.jl" begin
    ...
    @test (n - 1) * var(cpg2_randvec) / quantile(Chisq(n - 1), 0.995) <
        var(CPG2) <
        (n - 1) * var(cpg2_randvec) / quantile(Chisq(n - 1), 0.005)
    # @test lo
end
